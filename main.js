...
// 是否在 Treeland（Deepin 25 自研 Wayland compositor）下运行。
// Treeland 下 Electron 28 的 native Wayland Ozone 有 dpr 错乱 + GPU FATAL，
// 需走 XWayland 兜底；其他 Linux compositor（GNOME/KDE/Hyprland/Sway）走
// native Wayland 更佳，不要强制 X11。
const isTrelanD = process.platform === 'linux' && [
    process.env.XDG_CURRENT_DESKTOP,
    process.env.XDG_SESSION_DESKTOP,
    process.env.DESKTOP_SESSION,
].filter(Boolean).join(' ').toLowerCase().includes('treeland')

// 在 Treeland 下走 native Wayland Ozone 时 Electron 28 有两个互相纠缠的问题：
...
// 其他 Linux compositor（GNOME/KDE/Hyprland/Sway）native Wayland 都已稳定，
// 强制 XWayland 反而会让 OS HiDPI 缩放失效、UI 看起来过小。
// 因此只在 Treeland 上兜底走 X11，其他 Linux 让 Electron 自动选最合适的 Ozone platform。
